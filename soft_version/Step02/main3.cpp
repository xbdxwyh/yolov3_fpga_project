...
int main( int argc, char *argv[])
{
	...
	for(x=0;x<80;x++)//80 classe labels
	{
    	...
    	network *net = load_network("yolov3.cfg");
...
*/
    fread(yolo3_weight_buf, sizeof(float), 247583104/4, fp_w);
	fread(yolo3_beta_buf, sizeof(float), 108276/4, fp_b);
...
////////////////////////////////
	...
	layer_num = quantize_short16(yolo3_weight_buf,yolo3_weight_fixed_buf,yolo3_weight_offset,128,ap16_range,maxQ_array);
	for(i=0;i<layer_num;i++)
	{
		printf("[%d]=%d\n",i,maxQ_array[i]);
	}
	...
	layer_num = quantize_short16(yolo3_beta_buf,yolo3_weight_fixed_buf,yolo3_beta_offset,128,ap16_range,maxQ_array);
	for(i=0;i<layer_num;i++)
	{
		printf("[%d]=%d\n",i,maxQ_array[i]);
	}
